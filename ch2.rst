...
雖然 \ ``t``\  是 \ ``真``\  的預設表示法，任何不是 \ ``nil``\  的東西，在邏輯的語意中被認為是 \ ``真值``\  。
...
邏輯運算元 \ **and**\  和 \ **or**\  與條件式(conditionals)類似。兩者都接受任意數目的引數，但只對能夠決定回傳值的那幾個引數來作求值。如果所有的引數都為 \ ``真``\  （即不為 \ ``nil``\  )，那麼 \ ``and``\  返回最後一個引數的值：
...
如果其中一個引數為 \ ``假``\  ，那麼之後的所有引數都不會被求值。 \ ``or``\  也是如此，只要碰到一個是 \ ``真``\  的引數，就停止對之後的所有引數求值。
